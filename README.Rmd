...
The compute_mod_results() function works with a single caret model object and computes its performance results:

```{r}
compute_mod_results(glm_fit_1, "GLM 1") %>% head()
```

The all_mod_results() function works with multiple caret model objects and computes their model performance results:
...
The plot_mod_results() function produces a box plot of the models performance results:
